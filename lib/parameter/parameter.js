...
var Parameter = function (){
    ...
    this.makeText = function(text, code) {
        return function(){
            ...
            return {field: field, message: util.format.apply(util.format, input), code: code};
        }
    };
    ...
    this.customError = {
        ...
        notAllowedValue: self.makeText("the Parameter \'%s\' doesnt have a allowed value ", 400)
    };
    ...
    this.mongooseParser = function(model){
        ...
        if(tiper.getPrimitive(model) !== 'model' && self.isSchemaMongoose(model) ){
            ...
            //fieldSchema.required = model.required;23q `
    ...
    this.validate = function(fieldName, fieldValue,  validate, sucess, error){
        ...
        for(key in fieldValue){
            if(Object.keys(validate).indexOf(key) === -1){
                return error(self.customError.notAllowedValue(fieldName + '.' + key));
            }
        }
        ...
        for(var key in validate){
            ...
            if(fieldVal === undefined && !self.isSchema(validator)) continue;
    ...
    this.inputParserSchema = function(schema){
        ...
        if(tiper.is(schema, tiper.ARRAY)){
            ...
            for(var index in schema){
                tmp[index] = self.inputParserSchema(schema[index]);
            }
            return tmp;
        }
        ...
        if(!tiper.is(schema, tiper.OBJECT) && tiper.getPrimitive(schema) !== 'model'){
        ...
        if(tiper.getPrimitive(schema) === 'model'){
            ...
            tmp = self.mongooseParser(schema);
            iterator = tmp;
        }else{
            iterator = schema;
        }
        ...
        for(var key in iterator){
            tmp[key] = self.inputParserSchema(iterator[key]);
    ...
    this.inputParser = function(item){
        ...
        for(var index in item ){
            ...
            tmp[index] = {};
            tmp[index].field = item[index].field;
            tmp[index].schema = self.inputParserSchema(schema);

            if(tiper.is(tmp[index].Schema, tiper.OBJECT)){
