...
var Parameter = function (){
    ...
    this.validate = function(fieldName, fieldValue,  validate, sucess, error){
        ...
        if(!tiper.is(validate, tiper.OBJECT) && tiper.getPrimitive(validate) !== 'model') {
            ...
            console.log(tiper.get(validate));
            console.log(fieldValue);
            console.log(fieldName);

            if(tiper.is(validate, tiper.STRING) && tiper.get(fieldValue) !== '_TIPER_'+validate){

                return error(self.customError.notType(fieldName, validate));
            }
            else if(tiper.get(fieldValue) !== tiper.get(validate())){
